...
#endif
#define ALIGN0(X) ((X) & ~(sizeof(void *)-1))
#define ALIGN_UP(X) ALIGN0((X)+sizeof(void *)-1)
...
static void assert_valid_block(const heap_t *heap, const heap_block_t *block)
{
    ...
    if (heap < (const heap_t *)heap->last_block) {
        const heap_block_t *next __unused = get_next_block(block);
...
multi_heap_handle_t multi_heap_register_impl(void *start, size_t size)
{
    ...
    uintptr_t end = ALIGN0((uintptr_t)start + size);
    ...
    */
    heap->free_bytes = ALIGN0(size) - sizeof(heap_t) - sizeof(first_free_block->header) - sizeof(heap_block_t);
    heap->minimum_free_bytes = heap->free_bytes;
    heap->total_bytes = ALIGN0(size);
    return heap;
}
