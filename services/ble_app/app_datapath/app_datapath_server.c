...
extern void app_tws_connected_evt_handler(uint8_t conidx);
...
void app_datapath_server_disconnected_evt_handler(uint8_t conidx)
{
    if (conidx == app_datapath_server_env.connectionIndex) {
    	app_datapath_server_env.connectionIndex = GAP_INVALID_CONIDX;
...
void app_datapath_server_control_notification(bool isEnable, uint8_t conidx)
{
	struct ble_datapath_control_notification_t * req = KE_MSG_ALLOC(DATAPATHPS_CONTROL_NOTIFICATION,
                                                KE_BUILD_ID(prf_get_task_from_id(TASK_ID_DATAPATHPS), conidx),
                                                TASK_APP,
                              ...
                              ke_task_id_t const src_id)
{
	...
	if (param->isNotificationEnabled)
	{
		...
		uint32_t supervision_timeout_in_ms)
{
	...
	appm_update_param(app_datapath_server_env.connectionIndex, &conn_param);
}
