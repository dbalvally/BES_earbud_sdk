...
{
    app_ama_update_conn_parameter(conidx,AMA_BLE_CONNECTION_INTERVAL_MIN_IN_MS, 
...
void app_ama_disconnected_evt_handler(uint8_t conidx)
{
	if (conidx == app_ama_env.connectionIndex)
	{
		app_ama_env.connectionIndex = BLE_INVALID_CONNECTION_INDEX;
		app_ama_env.isCmdNotificationEnabled = false;
		app_ama_env.mtu[conidx] = 0;
	#ifdef __AMA_VOICE__
...
void app_ama_init(void)
{
	...
	app_ama_env.connectionIndex =  BLE_INVALID_CONNECTION_INDEX;
	app_ama_env.isCmdNotificationEnabled = false;	
	memset((uint8_t *)&(app_ama_env.mtu), 0, sizeof(app_ama_env.mtu));		
}
                              ...
                              ke_task_id_t const src_id)
{
	...
	if (app_ama_env.isCmdNotificationEnabled)
	{
		if (BLE_INVALID_CONNECTION_INDEX == app_ama_env.connectionIndex)
		{
			uint8_t conidx = KE_IDX_GET(src_id);
			app_ama_connected_evt_handler(conidx);

			if (app_ama_env.mtu[conidx] > 0)
			{
				app_ama_updata_mtu_size(app_ama_env.mtu[conidx]);
			}
		}
	}
#ifdef __AMA_VOICE__
...
void app_ama_update_conn_parameter(uint8_t conidx, uint32_t min_interval_in_ms, uint32_t max_interval_in_ms,
...
void app_ama_mtu_exchanged_handler(uint8_t conidx, uint16_t MTU)
{
	if (conidx == app_ama_env.connectionIndex)
	{
